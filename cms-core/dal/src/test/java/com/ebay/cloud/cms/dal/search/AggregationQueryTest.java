...
public class AggregationQueryTest extends SearchBaseTest {
    ...
    @Test
    public void test15ListField() {
        MetaClass metadata = raptorMetaService.getMetaClass("ServiceInstance");

        SearchGroup group = new SearchGroup();
        group.addGroupField(createGroupField(metadata, "manifestRef"));
        AggregationField count = new AggregationField(AggFuncEnum.COUNT, null);
        group.addAggregationField(count);
        ...
        for (IEntity entity : result.getResultSet()) {
            Assert.assertEquals(10, ((DBObject) entity.getNode()).get("_count"));
        }
    }

    @Test
    public void test16ListField_multi_group() {
        MetaClass metadata = raptorMetaService.getMetaClass("ServiceInstance");

        SearchGroup group = new SearchGroup();
        group.addGroupField(createGroupField(metadata, "manifestRef"));
        group.addGroupField(createGroupField(metadata, "activeManifestRef"));
        AggregationField count = new AggregationField(AggFuncEnum.COUNT, null);
        group.addAggregationField(count);
    ...
    @Test
    public void test17ListField_mix_group() {
        MetaClass metadata = raptorMetaService.getMetaClass("ServiceInstance");

        SearchGroup group = new SearchGroup();
        group.addGroupField(createGroupField(metadata, "manifestRef"));
        group.addGroupField(createGroupField(metadata, "name"));
        AggregationField count = new AggregationField(AggFuncEnum.COUNT, null);
        group.addAggregationField(count);
        ...
        SearchResult result = searchService.search(query, option, raptorContext);
        Assert.assertEquals(20, result.getResultSize());
        for (IEntity entity : result.getResultSet()) {
            Assert.assertEquals(1, ((DBObject) entity.getNode()).get("_count"));
        }
    }
