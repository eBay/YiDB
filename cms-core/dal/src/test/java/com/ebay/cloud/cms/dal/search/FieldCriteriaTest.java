...
public class FieldCriteriaTest extends SearchBaseTest {
    ...
    /**
     * The definition of NIN of array should be same as $eleMatch
     */
    @Test
    public void testArrayFieldNIN() {
        MetaClass metadata = raptorMetaService.getMetaClass("ServiceInstance");
        String fieldName = "manifestRef";
        MetaField field = metadata.getFieldByName(fieldName);
        List<Object> valueList = new ArrayList<Object>();
        valueList.add("manifest-1.0");
        valueList.add("manifest-3.0");
        SearchCriteria criteria = new FieldSearchCriteria(new SelectionField(field, strategy), FieldOperatorEnum.NIN,
                valueList);
        SearchProjection project = new SearchProjection();
        ISearchQuery query = new SearchQuery(metadata, criteria, project, strategy);
        SearchOption option = new SearchOption();
        SearchResult result = searchService.search(query, option, raptorContext);
        // no service instance has the manifest 1.0
...
//    }
    ...
    public void testEmbedNE02() {
        ...
        SearchProjection projection2 = new SearchProjection();
        MetaField metaField = metaref2.getRefMetaClass().getFieldByName("name");
        SearchCriteria criteria2 = new FieldSearchCriteria(metaField, strategy, FieldOperatorEnum.NE,
