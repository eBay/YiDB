...
public class TestDataLoader {
	...
	public static TestDataLoader getDataLoader(MongoDataSource dataSource, String repoName){
		...
		IPersistenceService persistService = new PersistenceServiceImpl(dataSource);
	 ...
	 * }
	 */
	public List<BsonEntity> load(List<BsonEntity> inputList, DBCollectionPolicy collectionPolicy){
		cleanUp();

		CheckConditions.checkNotNull(repo,"Failed to get meta data service from repository %s", repo.getRepositoryName());
		List<BsonEntity> loadList = new ArrayList<BsonEntity>();
		
		createDefaultBranch(repo);
	
		for(BsonEntity instNode: inputList){
		    String instName = "";
	...
	private String createDefaultBranch(Repository repo) {
		...
		ISearchQuery query = new SearchQuery(branchClass, null, searchProject, TestUtils.getDalSearchStrategy());
		SearchOption option = new SearchOption();
		SearchResult result = searchService.search(query, option, context);
		List<IEntity> bsonList = result.getResultSet();

		if (bsonList.size() == 0) {
			BsonEntity bsonEntity = new BsonEntity(branchClass);
	...
	public void cleanUp(){
        ...
        mainContext.setRegistration(TestUtils.getTestDalImplemantation(dataSource));
        ...
        histContext.setRegistration(TestUtils.getTestDalImplemantation(dataSource));
