...
package com.ebay.cloud.cms.dalapi.entity.visitor;
...
public abstract class AbstractBsonValidator implements IEntityVisitor {
    ...
    public AbstractBsonValidator(AbstractEntityIDHelper helper) {
        this.helper = helper;
    }
	...
	public void processAttribute(IEntity currentEntity, MetaField metaField) {
		...
		if (fieldEnum != null) {
		    processInternalAttribute(fieldEnum, (BsonEntity)currentEntity, parentField, (BsonEntity)parentEntity);		    		         
		} else {
		    processUserAttribute((BsonEntity)currentEntity, metaField);
        }
	}
	...
	protected void processInternalAttribute(InternalFieldEnum fieldEnum, BsonEntity currentEntity, MetaRelationship refField, BsonEntity parentEntity) {
	    switch (fieldEnum) {
            case ID:
	...
	public void processReference(IEntity currentEntity,MetaRelationship metaRelationship) {
	    ...
	    if (((BsonEntity)currentEntity).getFieldTimestamp(fieldName) == null) {
            ((BsonEntity)currentEntity).setFieldTimestamp(fieldName, visitTime);
        }
	    ...
	    List<BsonEntity> refEntities = (List<BsonEntity>) currentEntity.getFieldValues(fieldName);
        for (BsonEntity refEntity : refEntities) {
            if (refEntity == null) {
                continue;
            }
...
//                        })));
		...
		if (metaRelationship.getCardinality() == CardinalityEnum.Many) {
		    ((BsonEntity)currentEntity).setFieldLength(fieldName);
        }
	}
    
	protected abstract void processId(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity);
	
	protected abstract void processVersion(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity);
	
	protected abstract void processParentVersion(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity);

	protected abstract void processStatus(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity);

	protected abstract void processCreateTime(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity);

	protected abstract void processCreator(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity);

    protected void processUser(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity) { }

    protected void processModifier(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity) { }

    protected void processComment(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity) { }

    protected void processShardKey(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity) { }

    protected void processHostEntity(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity) { }

    protected void processMetaVersion(BsonEntity parentEntity, MetaRelationship refField, BsonEntity currentEntity) {
