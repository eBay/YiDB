...
window.CodeMirror = (function() {"use strict";
    ...
    function CodeMirror(place, givenOptions) {
        ...
        var shiftSelecting, lastClick, lastDoubleClick, lastScrollTop = 0, draggingText, overwrite = false, suppressEdits = false;
        ...
        var instance = wrapper.CodeMirror = {
            ...
            setOption : function(option, value) {
                ...
                if(option == "mode" || option == "indentUnit")
                    ...
                    keyMapChanged();
                if(option == "lineNumbers" || option == "gutter" || option == "firstLineNumber" || option == "theme" || option == "lineNumberFormatter") {
                    gutterChanged();
                    updateDisplay(true);
                }
            },
            ...
            getHistory : function() {
                history.time = 0;
                return {
                    done : history.done.concat([]),
                    undone : history.undone.concat([])
                };
            },
            ...
            addWidget : function(pos, node, scroll, vert, horiz) {
                ...
                if(vert == "over")
                    ...
                    var vspace = Math.max(scroller.offsetHeight, doc.height * textHeight()), hspace = Math.max(sizer.clientWidth, lineSpace.clientWidth) - paddingLeft();
            ...
            scrollTo : function(x, y) {
                ...
                if(y != null)
                    scrollbar.scrollTop = scroller.scrollTop = y;
                updateDisplay([]);
            },
            ...
            setSize : function(width, height) {
                ...
                if(width != null)
                    wrapper.style.width = interpret(width);
                if(height != null)
                    scroller.style.height = interpret(height);
                instance.refresh();
            },
            ...
            function doSelect(cur) {
                if(type == "single") {
                    ...
                    if(posLess(cur, startstart))
                        setSelectionUser(word.from, startend);
                    else
                        setSelectionUser(startstart, word.to);
                } else if(type == "triple") {
                    if(posLess(cur, startstart))
                        setSelectionUser(startend, clipPos({
                            line : cur.line,
                            ch : 0
                        }));
                    else
                        setSelectionUser(startstart, clipPos({
                            line : cur.line + 1,
                            ch : 0
                        }));
                }
        ...
        function handleKeyBinding(e) {
            ...
            if(e_prop(e, "shiftKey")) {
                handled = lookupKey("Shift-" + name, options.extraKeys, options.keyMap, function(b) {
        ...
        function scrollCursorIntoView() {
            ...
            if(coords.y + box.top < 0)
                doScroll = true;
            else if(coords.y + box.top + textHeight() > (window.innerHeight || document.documentElement.clientHeight))
                doScroll = false;
            if(doScroll != null) {
        ...
        function updateSelection() {
            ...
            if(collapsed) {
                ...
                var add = function(left, top, right, height) {
                    ...
                    fragment.appendChild(elt("div", null, "CodeMirror-selected", "position: absolute; left: " + left + "px; top: " + top + "px; " + rstyle + "; height: " + height + "px"));
                };
        ...
        function onContextMenu(e) {
            ...
            function rehide() {
                ...
                if(newVal != val && !options.readOnly)
                    operation(replaceSelection)(newVal, "end");
                inputDiv.style.position = "relative";
                input.style.cssText = oldCSS;
                if(ie_lt9)
                    scrollbar.scrollTop = scrollPos;
        ...
        function matchBrackets(autoclear) {
            ...
            var one = markText({
                ...
                line : head.line,
                ch : pos + 1
            }, style), two = found.pos != null && markText({
                line : i,
                ch : found.pos
            }, {
                line : i,
        ...
        function endOperation() {
            ...
            var sc = selectionChanged, cbs = callbacks;
            // these can be reset by callbacks
    ...
    Line.prototype = {
            ...
            for(var i = 0; i < mk.length; ++i) {
                ...
                if(close && omk) {
                    for(var j = 0; j < omk.length; ++j) {
                        var om = omk[j];
                        if(!om.sameSet(mark) || om.from != null)
                            continue;
        ...
        getElement : function(makeTab, wrapAt, wrapWBR) {
            ...
            function span_(html, text, style) {
                ...
                if(style)
                    html.appendChild(elt("span", [content], style));
                else
    ...
    History.prototype = {
        addChange : function(start, added, old) {
            ...
            if(this.compound && cur && !this.closed) {
            ...
            } else if(dtime > 400 || !last || this.closed || last.start > start + old.length || last.start + last.added < start) {
                this.done.push([{
                    start : start,
                    added : added,
                    old : old
                ...
                var startBefore = Math.max(0, last.start - start), endAfter = Math.max(0, (start + old.length) - (last.start + last.added));
