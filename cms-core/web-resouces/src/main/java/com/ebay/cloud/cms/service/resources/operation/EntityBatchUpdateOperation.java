...
public class EntityBatchUpdateOperation extends EntityBatchOperation {

    public EntityBatchUpdateOperation(CMSServer server, String mode, String failReturnOptionVal, HttpServletRequest request, String priority,
        ...
        super(server, mode, failReturnOptionVal, request, priority, consistPolicy, repoName, metadata, branch, oid, jsonBody, errorMsg,
                uriInfo);
    }

    @Override
    protected void performBatchAction(EntityContext context, List<IEntity> entities, List<String> parseFails) {
        context.setModifyAction(ModifyAction.MODIFYENTITY);
        cmsServer.batchUpdate(priority, entities, context, parseFails);
    }
