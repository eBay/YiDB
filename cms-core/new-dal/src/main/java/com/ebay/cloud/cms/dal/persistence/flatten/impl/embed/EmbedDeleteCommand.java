...
public class EmbedDeleteCommand extends AbstractEmbedCommand implements IPersistenceCommand {
    ...
    public EmbedDeleteCommand(NewBsonEntity embedEntity, FlattenEntityIDHelper helper) {
    ...
    public void execute(PersistenceContext context) {
        ...
        if(rootObject == null){
            throw new CmsDalException(DalErrCodeEnum.ENTITY_NOT_FOUND,"Embed Delete, parenet document doesn't exist!  " + parentId);
        }
        ...
        if (result.getN() == 0) {
            ...
            throw new CmsDalException(DalErrCodeEnum.VERSION_CONFLICT,"Version check fails for " + entityId + " in class " + metaClass.getName() + "! entity is " + entity.toString());
        }
    }
    ...
    public DBObject buildDeleteBody(MetaRelationship metaField, String parentPath, String entityId, int newVersion, MetaClass rootMetaClass) {
        String embedPath = metaField.getFlattenValueDbName();
        String embedLengthPath = metaField.getFlattenPropertyValueDbName(FieldProperty.LENGTH);
        if (!StringUtils.isNullOrEmpty(parentPath)) {
            embedPath = parentPath + FlattenEntityIDHelper.DOT + embedPath;
            embedLengthPath = parentPath + FlattenEntityIDHelper.DOT + embedLengthPath;
        }
